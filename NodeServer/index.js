...
var URIs = {
  ...
  URI_GET_LIST_OF_INTEREST_LOCATIONS: "/API/getListOfInterestLocations",
 ...
 */
app.post(URIs.URI_GET_LIST_OF_INTEREST_LOCATIONS, function(req, res) {
  ...
  try {
    ...
    var email = req.body.email;
    var password = req.body.password;
    var available_locations = req.body.available_locations;
    ...
    databaseManager.getListOfInterestLocations(account_id, email, password, available_locations, function(isSuccess, returned_list) {
      ...
      res.send(JSON.stringify({location_list:JSON.stringify(returned_list)}));
    });
  } catch(error) {
    console.log("Unknown error occurred");
  }
